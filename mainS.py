...
for rep in range(1,2):
    for split in range(0,5):
        ...
        path_X_train_S2 = f"/mnt/DATA/JE/{site}/{sensor}/Training/{sensor}_Training_split_{split}.npy"

        path_Y_valid = f"/mnt/DATA/JE/{site}/Ground_truth/Validation/Ground_truth_Validation_split_{split}.npy"
        path_X_valid_S2 = f"/mnt/DATA/JE/{site}/{sensor}/Validation/{sensor}_Validation_split_{split}.npy"

        path_Y_test = f"/mnt/DATA/JE/{site}/Ground_truth/Test/Ground_truth_Test_split_{split}.npy"
        path_X_test_S2 = f"/mnt/DATA/JE/{site}/{sensor}/Test/{sensor}_Test_split_{split}.npy"
        ...
        train(Model_S2(n_classes=11),train_loader,valid_loader,test_loader,num_epochs=300,csv_name=f"{sensor}_withrelu_site-{site}_rep-{rep}_result_split-{split}.csv",model_file=f"model-S1_split-{split}.pth")
