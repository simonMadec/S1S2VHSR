...
for index,reg_ in df.iterrows():
    ...
    idd = reg_["id"]
    for method in ["model"]: #"v2branchSplit2","v2branch"
        for ref in ["Spot","S1"]:
            for dataset in ["data_reunion_origin"]: # ,]: # "data_ToyReunion","data_ToyDordogne",,"data_dordogne_origin","data_ToyDordogne"
                namemodel = f"{method} 2505 inf {dataset} ref {ref} model 1 5m"
                for sensor in [["S1","S2","Spot"]]: #["S2"],["S1"],["Spot","S2"],["S1","S2","Spot"],["Spot","S2","S1"],["S1"],["S1","S2"],["Spot"],["S2"]]:
                    ...
                    with torch.no_grad():
                        ...
                        predict_dataset = Dataset_inference(root=root,dataset=dataset,bbox=bbox,sensor=sensor,ref=ref)
                        ...
                        outname = f"InfMap/{dd} Inf {method} {dataset} id {idd} {'-'.join(sensor)} bbox {namemodel}"
