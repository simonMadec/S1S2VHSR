...
class DatasetS1S2VHSRbig2(BaseDataset):
    """Read mutiples numpy
        Used for training ..
    ...
    def __init__(
        ...
        if "reunion" in Path(root).stem:
            self.num_target = 11
            size = 880723
            if dataset in ["Validation","Test"]:
                size = int(880723*0.33)
            self.list_y_numpy_dir = random.choices(list_y_numpy_dir, k=int(size/256))

        elif "dordogne" in Path(root).stem:
            self.num_target = 7
            size = 814436
            if dataset in ["Validation","Test"]:
                size = int(814436*0.33)
            self.list_y_numpy_dir = random.choices(list_y_numpy_dir, k=int(size/256))
        else:
        ...
        # for x in self.sensor:
        #     if x == "S1":
        #         data = self.data_memmaps_S1[memmap_index][index_in_memmap].transpose(2,0,1)
        #         dicts[x] = torch.as_tensor(np.array(data).astype('float32'))
        #     elif x == "S2":
        #         data = self.data_memmaps_S2[memmap_index][index_in_memmap]
        #         center = int(data.shape[1]/2)
        #         dicts[x] = torch.as_tensor(np.array( data[center,center,:].reshape(21,6).transpose(1,0)).astype('float32'))
        #     elif x == "Spot":
        #         data = self.data_memmaps_PAN[memmap_index][index_in_memmap].transpose(2,0,1)[:,:-1,:-1]
        #         dicts["PAN"] = torch.as_tensor(np.array(data).astype('float32'))
        #         data = self.data_memmaps_MS[memmap_index][index_in_memmap].transpose(2,0,1)[:,:-1,:-1]
        #         dicts["MS"] = torch.as_tensor(np.array(data).astype('float32'))
    ...
    def __getitem__(self, index):
        ...
        for x in self.sensor:
            if x == "S1":
                ...
                # center = int(data.shape[1]/2)
                # dicts[x] = torch.as_tensor(np.array( data[center,center,:].reshape(int(data.shape[2]/6),6).transpose(1,0)).astype('float32'))
            elif x == "Spot":
                # data = self.data_memmaps_PAN[memmap_index][index_in_memmap].transpose(2,0,1)[:,:-1,:-1]
                # dicts["PAN"] = torch.as_tensor(np.array(data).astype('float32'))
                # data = self.data_memmaps_MS[memmap_index][index_in_memmap].transpose(2,0,1)[:,:-1,:-1]
